...
pub async fn user_repository_contract<R: UserRepository>(repo: R) {
    ...
    assert!(repo.list_library_access(&UserId("nope".into())).await.unwrap().is_empty());

    repo.create(user("admin", "admin", Role::Admin, ts(1))).await.unwrap();
    repo.create(user("u1", "alice", Role::User, ts(2))).await.unwrap();
    ...
    assert_eq!(alice.preferred_audio, vec![LanguageCode("en".into()), LanguageCode("de".into())]);
    assert_eq!(
    ...
    assert_eq!(listed.items.iter().map(|u| u.id.0.as_str()).collect::<Vec<_>>(), ["admin", "u1"]);
    ...
    .await
    .unwrap();
    let access = repo.list_library_access(&UserId("u1".into())).await.unwrap();
    assert_eq!(access.len(), 2);
    assert!(access.iter().all(|a| a.user == UserId("u1".into())));

    repo.set_library_access(&UserId("u1".into()), &[LibraryId("lib3".into())]).await.unwrap();
    let access = repo.list_library_access(&UserId("u1".into())).await.unwrap();
