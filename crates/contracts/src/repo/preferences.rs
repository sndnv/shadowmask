...
pub async fn preferences_repository_contract<R: PreferencesRepository>(repo: R) {
    ...
    assert!(
        repo.get_subtitle_offset(&u1, &v1, &embedded)
            .await
            .unwrap()
            .is_none()
    );
    ...
    repo.add_watchlist(watchlist("u1", movie("m2"), 20))
        .await
        .unwrap();
    repo.add_watchlist(watchlist("u1", episode("e1"), 10))
        .await
        .unwrap();
    repo.add_watchlist(watchlist("u1", movie("m1"), 10))
        .await
        .unwrap();
    ...
    repo.add_watchlist(watchlist("u1", movie("m1"), 99))
        .await
        .unwrap();
    let wl = repo.list_watchlist(&u1).await.unwrap();
    assert_eq!(wl.len(), 3);
    ...
    let ids: Vec<_> = repo
        ...
        .unwrap()
        .iter()
        .map(|i| i.title.id().to_owned())
        .collect();
    assert_eq!(ids, ["e1", "m2"]);

    repo.add_favorite(favorite("u1", movie("m1"), 5))
        .await
        .unwrap();
    repo.add_favorite(favorite("u1", episode("e2"), 5))
        .await
        .unwrap();
    ...
    repo.add_favorite(favorite("u1", movie("m1"), 7))
        .await
        .unwrap();
    ...
    repo.set_subtitle_offset(offset("u1", "v1", embedded.clone(), -250))
        .await
        .unwrap();
    repo.set_subtitle_offset(offset("u1", "v1", file.clone(), 500))
        .await
        .unwrap();
    let got = repo
        .get_subtitle_offset(&u1, &v1, &embedded)
        .await
        .unwrap()
        .unwrap();
    ...
    let got_file = repo
        .get_subtitle_offset(&u1, &v1, &file)
        .await
        .unwrap()
        .unwrap();
    ...
    repo.set_subtitle_offset(offset("u1", "v1", embedded.clone(), 999))
        .await
        .unwrap();
    assert_eq!(
        repo.get_subtitle_offset(&u1, &v1, &embedded)
    ...
    assert!(
        repo.get_subtitle_offset(&u1, &VersionId("v2".into()), &embedded)
            .await
            .unwrap()
            .is_none()
    );
    ...
    assert!(
        repo.get_subtitle_offset(&u2, &v1, &embedded)
            .await
            .unwrap()
            .is_none()
    );
    repo.add_watchlist(watchlist("u2", movie("m9"), 1))
        .await
        .unwrap();
