...
fn cleaned(value: Option<String>) -> Option<String> {
    value.as_deref().map(str::trim).filter(|value| !value.is_empty()).map(str::to_owned)
}
...
fn imdb_of(ids: &[domain::metadata::ExternalId]) -> Option<String> {
    ids.iter().find(|id| id.source == "imdb").map(|id| id.value.clone())
}
...
async fn title_context<C: CatalogRepository + Send + Sync>(
    ...
    match title {
        ...
        TitleId::Episode(id) => {
            ...
            let season = catalog.get_season(&episode.season).await.map_err(internal)?;
            let series = match &season {
                Some(season) => catalog.series_detail(&season.series).await.map_err(internal)?,
                None => None,
            };
            Ok(TitleContext {
                imdb_id: series.as_ref().map(|s| imdb_of(&s.external_ids)).unwrap_or_default(),
    ...
    P: SubtitleProvider + Send + Sync,
{
    ...
    let fetched = match state.provider.download(file_id).await {
        ...
        Err(err) => {
            debug!("subtitle download failed for version [{}]: {err}", version.0);
            return Err(ApiError::bad_gateway("subtitle provider error"));
        }
    };
    let path = state
    ...
    S: SubtitleReader + Send + Sync,
{
    ...
    let content = state.subtitles.load(&file.path).await.map_err(|_| ApiError::internal())?;
    debug!("user [{actor}] read subtitle [{subtitle}] of version [{}]", version.0);
    ...
    S: Send + Sync,
{
    ...
    if !is_managed(target.source) {
        return Err(ApiError::forbidden("only downloaded or generated subtitles can be renamed"));
    }
    ...
    S: SubtitleStore + Send + Sync,
{
    ...
    debug!("user [{actor}] deleted subtitle [{subtitle}] of version [{}]", version.0);
    Ok(StatusCode::NO_CONTENT)
}
