...
use domain::catalog::MovieDetail;
...
impl From<MovieDetail> for MovieDetailResponse {
    fn from(d: MovieDetail) -> Self {
        let movie = d.movie;
        MovieDetailResponse {
            id: movie.id.0,
            title: movie.title,
            ...
            content_rating: movie.content_rating.map(Into::into),
            manually_edited: movie.manually_edited,
            added_at: movie.added_at.to_string(),
            updated_at: movie.updated_at.to_string(),
            artwork: ArtworkDto::from_refs(movie.artwork),
