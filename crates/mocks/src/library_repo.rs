...
pub struct MockLibraryRepo {
    ...
    scan_progress: Arc<Mutex<Vec<f32>>>,
...
impl LibraryRepository for MockLibraryRepo {
    ...
    async fn reconcile_duplicates(
        ...
        self.duplicates
            .lock()
            .unwrap()
            .retain(|id, (owner, _, _)| owner != library || detected.contains(id));
        Ok(())
    }
    ...
    async fn reconcile_unmatched(
        ...
        self.unmatched
            .lock()
            .unwrap()
            .retain(|_, (file, _)| &file.library != library || present_paths.contains(&file.path));
        Ok(())
    }
}
