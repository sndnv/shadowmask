...
impl MockAuthTokenRepo {
    ...
    fn guard(&self) -> Result<(), RepositoryError> {
        if self.fail.load(Ordering::Relaxed) {
            Err(RepositoryError::Backend("mock auth token failure".to_owned()))
        } else {
            Ok(())
        }
    }
...
impl AuthTokenRepository for MockAuthTokenRepo {
    ...
    async fn store_refresh(&self, session: AuthSession) -> Result<(), RepositoryError> {
        self.guard()?;
        self.state.lock().unwrap().refresh.insert(session.id.clone(), session);
        Ok(())
    }
    ...
    async fn store_api_token(&self, token: ApiToken) -> Result<(), RepositoryError> {
        self.guard()?;
        self.state.lock().unwrap().api_tokens.insert(token.token_hash.clone(), token);
        Ok(())
    }
    ...
    async fn touch_api_token(
        ...
        self.guard()?;
        if let Some(token) =
            self.state.lock().unwrap().api_tokens.values_mut().find(|token| &token.id == id)
        {
    ...
    async fn revoke_api_token(&self, id: &ApiTokenId) -> Result<(), RepositoryError> {
        self.guard()?;
        self.state.lock().unwrap().api_tokens.retain(|_, token| &token.id != id);
        Ok(())
    }
}
...
mod tests {
    ...
    async fn list_and_revoke_scoped_by_user() {
        ...
        repo.revoke_api_token(&ApiTokenId("t1".into())).await.unwrap();
    ...
    async fn device_and_api_token_round_trip() {
        let repo = MockAuthTokenRepo::new();
        repo.upsert_device(device("d1")).await.unwrap();
        assert_eq!(repo.get_device(&DeviceId("d1".into())).await.unwrap().unwrap().name, "Roku");
        assert!(repo.get_device(&DeviceId("missing".into())).await.unwrap().is_none());

        repo.store_api_token(api_token("t1", "hash-1")).await.unwrap();
        assert_eq!(repo.find_api_token_by_hash("hash-1").await.unwrap().unwrap().id.0, "t1");
    ...
    async fn refresh_store_find_revoke() {
        ...
        assert!(repo.find_refresh(&AuthSessionId("j1".into())).await.unwrap().is_some());
    ...
    async fn link_code_redeemed_once_and_respects_expiry() {
        ...
        assert!(repo.redeem_link_code("MISSING", now).await.unwrap().is_none());
    }
    ...
    async fn lists_live_codes_per_user_and_deletes_scoped_to_owner() {
        ...
        assert_eq!(repo.list_link_codes(&UserId("u1".into()), now).await.unwrap().len(), 1);
        repo.delete_link_code("LIVE", &UserId("u1".into())).await.unwrap();
        assert!(repo.list_link_codes(&UserId("u1".into()), now).await.unwrap().is_empty());
    }
    ...
    async fn surfaces_backend_failure() {
        ...
        assert!(
            repo.touch_api_token(&ApiTokenId("t1".into()), Timestamp::UNIX_EPOCH).await.is_err()
        );
