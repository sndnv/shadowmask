...
struct State {
    ...
    rebuilds: u32,
    fail: bool,
    last_filter: Option<TitleListFilter>,
}
...
impl MockSearchIndex {
    ...
    pub fn last_filter(&self) -> Option<TitleListFilter> {
        self.state.lock().unwrap().last_filter.clone()
    }
}
...
impl SearchIndex for MockSearchIndex {
    async fn search(
        ...
        state.last_filter = Some(filter.clone());
        if state.fail {
...
mod tests {
    ...
    fn movie(title: &str) -> SearchResult {
        SearchResult::Movie(Movie {
            ...
            sort_title: domain::text::sort_title(title, &[]),
            year: None,
            overview: None,
