...
pub fn continue_watching(progress: &[PlaybackProgress]) -> Vec<PlaybackProgress> {
    let mut items: Vec<PlaybackProgress> =
        progress.iter().filter(|p| p.position_ms > 0).cloned().collect();
...
pub fn drop_resumable(
    ...
    let resumable: HashSet<&str> =
        in_progress.iter().map(|item| item.progress.version.0.as_str()).collect();
    now_playing.into_iter().filter(|n| !resumable.contains(n.session.version.0.as_str())).collect()
}

pub fn drop_unstarted(now_playing: Vec<NowPlaying>) -> Vec<NowPlaying> {
    now_playing.into_iter().filter(|n| n.card.progress_percent > 0).collect()
}
...
mod tests {
    ...
    fn orders_started_by_recency() {
        let entries =
            [progress("older", 50, 10), progress("newest", 50, 30), progress("middle", 50, 20)];
    ...
    fn item(version: &str) -> ContinueWatchingItem {
        ContinueWatchingItem { progress: progress(version, 400, 10), card: card() }
    }
    ...
    fn a_version_with_stored_progress_is_not_also_reported_as_playing() {
        ...
        assert_eq!(
            rows.iter().map(|n| n.session.version.0.clone()).collect::<Vec<String>>(),
            vec!["v2"]
        );
    }
    ...
    fn a_session_sitting_at_the_start_never_reaches_the_rail() {
        let stalled = NowPlaying { card: card_at(0), ..playing("v1") };
    ...
    fn several_stale_sessions_for_one_version_all_go() {
        let in_progress = [item("v1")];
        let rows = drop_resumable(vec![playing("v1"), playing("v1"), playing("v1")], &in_progress);
