...
pub enum MetadataJobPayload {
    Title { title: TitleRef, external_id: Option<ExternalId>, force: bool },
    People { ids: Vec<PersonId>, force: bool },
}
...
enum Wire {
    Title { title: TitleWire, external_id: Option<ExternalIdWire>, force: bool },
    People { ids: Vec<String>, force: bool },
}
    ...
    fn from(payload: &MetadataJobPayload) -> Self {
        match payload {
            ...
            MetadataJobPayload::People { ids, force } => {
                Wire::People { ids: ids.iter().map(|id| id.0.clone()).collect(), force: *force }
            }
        }
    }
}
...
impl From<Wire> for MetadataJobPayload {
    fn from(wire: Wire) -> Self {
        match wire {
            Wire::Title { title, external_id, force } => {
                let title = match title {
                ...
                MetadataJobPayload::Title {
                    ...
                    external_id: external_id
                        .map(|id| ExternalId { source: id.source, value: id.value }),
                    force,
                }
            }
            Wire::People { ids, force } => {
                MetadataJobPayload::People { ids: ids.into_iter().map(PersonId).collect(), force }
            }
        }
    }
}
...
mod tests {
    ...
    fn round_trips_people() {
        ...
        round_trip(MetadataJobPayload::People { ids: Vec::new(), force: false });
    }
